...
## Graph
Since it is always a ULM in each time intervals then in a $x-t$ graph the curve is a straight line in each interval. Fig. <a href="#fig:sfulm-1d-ABCDE-x-t">2</a> wil show the graph.

<oo>
svg 600 200 #fafafa fig:sfulm-1d-ABCDE-x-t|A $x-t$ graph for the particle visiting $A$, $B$, $C$, $D$, and $E$ points.
